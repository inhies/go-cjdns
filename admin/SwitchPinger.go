...
type SwitchPinger struct{ client *Client }
...
func (s *SwitchPinger) Ping(path, dataIn string, timeout int) (dataOut string, ms int, err error) {
	var (
		args = &struct {
	...
	if pack, err = s.client.sendCmd(&request{AQ: "SwitchPinger_ping", Args: args}); err == nil {
