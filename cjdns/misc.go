...
// just crashes it.
func (a *Admin) Memory() (memory int64, err error) {
	response, err := SendCmd(a, "memory", nil)
	if err != nil {
		return
	}
...
func (a *Admin) NodeStore_dumpTable(page int) (response map[string]interface{}, err error) {
	args := make(map[string]interface{})
	args["page"] = page
	response, err = SendCmd(a, "NodeStore_dumpTable", args)
	if err != nil {
		return
	}
	return
...
func (a *Admin) ReqCookie() (cookie string, err error) {
	response, err := SendCmd(a, "cookie", nil)
	if err != nil {
		return
	}
...
func (a *Admin) SendPing(timeout time.Duration) (bool, error) {
	rChan := make(chan map[string]interface{}, 1)
	go func() {
		response, err := SendCmd(a, "ping", nil)
		if err != nil {
			return
		}
		rChan <- response
